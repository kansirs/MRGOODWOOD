...
router.get('/leads.csv', (req, res) => {
  ...
  const headers = ['id', 'created_at', 'name', 'phone', 'email', 'town', 'project_type', 'timeline', 'message', 'status'];
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="goodwood-leads.csv"');
  res.send(rows.join('\n'));
});
